...
export default function SuggestionPills({ onSelect }: SuggestionPillsProps) {
  return (
    <div className="w-full max-w-[740px] mx-auto mt-3.5 sm:mt-7">
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-3 mb-2 sm:mb-3.5">
        ...
        <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 dark:text-neutral-500 tracking-wider uppercase">Try asking</span>
        <div className="h-[1px] w-8 sm:w-12 bg-slate-200 dark:bg-neutral-800" />
      </div>
        ...
        {FEATURED_SUGGESTIONS.map((item, idx) => {
          ...
          return (
            <motion.button
              ...
              "
            >
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 transition-colors ${item.iconBg}`}>
                <Icon size={14} className="sm:w-4 sm:h-4" />
              </div>
              <div className="min-w-0 flex-1 leading-tight">
                <div className="text-[10.5px] sm:text-[11.5px] font-semibold text-slate-800 dark:text-neutral-200 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  {item.line1}
                </div>
